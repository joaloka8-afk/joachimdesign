...
const GlobalStyle = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=Instrument+Serif:ital@0;1&family=JetBrains+Mono:wght@300;400;500&family=Caveat:wght@400;600&display=swap');

    :root {
      --paper: #F5F1E8;
      --paper-warm: #EFE9DC;
      --ink: #14120F;
      --ink-muted: rgba(20, 18, 15, 0.58);
      --ink-subtle: rgba(20, 18, 15, 0.38);
      --ink-faint: rgba(20, 18, 15, 0.14);
    ...
    html, body, #root { background: var(--paper); color: var(--ink); }
    body { font-family: 'Inter', system-ui, sans-serif; -webkit-font-smoothing: antialiased; }

    .font-display { font-family: 'Instrument Serif', Georgia, serif; letter-spacing: -0.02em; font-weight: 400; }
    ...
    .animate-blink      { animation: blink-caret 1s steps(2, end) infinite; }
    ...
    .reveal-line { display: inline-block; transform: translateY(105%); }
    .reveal-line.is-visible {
      animation: line-reveal 1.05s cubic-bezier(.2,.85,.2,1) forwards;
    }
    ...
    ::-webkit-scrollbar-thumb { background: #D4CEBE; }
...
const RevealLines = ({ lines, delay = 0, lineDelay = 110, className = '' }) => {
  ...
  <header className="fixed top-0 inset-x-0 z-50 border-b border-[var(--border)] bg-[var(--paper)]/85 backdrop-blur-md">
    <div className="max-w-[1200px] mx-auto px-6 lg:px-10 h-14 flex items-center justify-between">
      ...
      <nav className="hidden md:flex items-center gap-7 text-[13px] text-[var(--ink-muted)]">
        {['Work', 'Services', 'Process', 'About', 'Notes'].map((item) => (
          <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-[var(--ink)] transition-colors">
            {item}
          </a>
        ))}
      </nav>
...
const PortraitCard = () => (
  <div className="relative" style={{ transform: 'rotate(1.5deg)' }}>
    ...
    <div className="bg-[var(--paper-warm)] border border-[var(--border-strong)] p-4 pb-6 shadow-[4px_8px_0_rgba(20,18,15,0.08)] hover:shadow-[6px_12px_0_rgba(20,18,15,0.12)] hover:-translate-y-0.5 transition-all duration-500">
      {/* "Photo" — SVG illustration since we can't use stock images */}
        ...
        <div className="flex gap-8 animate-marquee whitespace-nowrap items-center">
          {row.map((c, i) => (
            <React.Fragment key={i}>
              <span className="font-display italic text-[26px] text-[var(--ink)]/75">
                {c}
              </span>
...
const ServiceRow = ({ no, icon: Icon, title, price, desc, includes, timeline }) => (
  <div className="bg-[var(--paper)] p-8 lg:p-12 group hover:bg-[var(--paper-warm)] transition-colors duration-500 relative overflow-hidden">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
      ...
      <div className="lg:col-span-3">
        <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--ink-subtle)] mb-3">
          Includes
        </div>
    ...
    <section id="work" className="px-6 lg:px-10 py-24 lg:py-32 border-t border-[var(--border)]">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="border-t border-[var(--border)]">
          {projects.map((p, i) => (
            <Reveal key={p.name} delay={i * 50}>
              ...
              >
                <span className="col-span-2 lg:col-span-1 font-mono text-[12px] text-[var(--ink-subtle)] tabular-nums">
                  {p.year}
                </span>
                <span className="col-span-6 lg:col-span-4 font-display text-[24px] lg:text-[32px] leading-none transition-transform duration-300" style={{ transform: hovered === i ? 'translateX(8px)' : 'translateX(0)' }}>
                  {p.name}
                </span>
                <span className="col-span-12 lg:col-span-4 text-[13px] text-[var(--ink-muted)] italic font-display">
                  {p.kind}
                </span>
        ...
        <Reveal delay={200} className="mt-12 flex items-center gap-3">
          <span className="font-hand text-[20px] text-[var(--ink-muted)] -rotate-2">
            plus ~30 more
          </span>
          <span className="text-[var(--pink)] text-[16px]">→</span>
          <a href="#contact" className="text-[14px] text-[var(--ink)] link-underline">
            Ask me about a specific kind of project
          </a>
        </Reveal>
      </div>
    </section>
        ...
        {steps.map((s, i) => (
          <Reveal key={s.n} delay={i * 90}>
            <div className="bg-[var(--paper)] p-6 lg:p-7 h-full group hover:bg-[var(--paper-warm)] transition-colors duration-500 min-h-[260px] flex flex-col">
              <div className="font-display text-[56px] leading-none text-[var(--ink)]/25 group-hover:text-[var(--pink)] transition-colors duration-500 mb-8">
                {s.n}
              </div>
...
const SlackQuote = ({ author, role, time, text, reaction }) => (
  <div className="bg-[var(--paper-warm)] border border-[var(--border)] p-5 hover:-translate-y-1 hover:shadow-[4px_8px_0_rgba(20,18,15,0.08)] transition-all duration-500 group">
    ...
    <p className="text-[14px] leading-[1.6] text-[var(--ink)]/90 mb-4">
      {text}
    </p>
    <div className="inline-flex items-center gap-1.5 bg-[var(--paper)] border border-[var(--border)] rounded-full px-2 py-0.5 text-[12px] group-hover:scale-110 transition-transform duration-300 origin-left">
      <span>{reaction}</span>
      <span className="font-mono text-[10px] text-[var(--ink-muted)]">{Math.floor(Math.random() * 4) + 2}</span>
    </div>
  </div>
);
  ...
  <section id="notes" className="px-6 lg:px-10 py-24 lg:py-32 border-t border-[var(--border)]">
    <div className="max-w-[1200px] mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
        <div className="lg:col-span-5">
          <Reveal className="font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--pink)] mb-6">
            [ /now ]
          </Reveal>
          ...
          {faqs.map((f, i) => (
            <Reveal key={i} delay={i * 60}>
              ...
              >
                ...
                >
                  <p className="text-[15px] leading-[1.65] text-[var(--ink)]/75 max-w-[620px]">
                    {f.a}
                  </p>
                </div>
              </button>
    ...
    <section id="contact" className="px-6 lg:px-10 py-28 lg:py-40 border-t border-[var(--border)] relative overflow-hidden">
      ...
      <div className="max-w-[1000px] mx-auto relative">
        ...
        <Reveal delay={400} className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          {/* Email */}
          <div className="group">
            <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--ink-subtle)] mb-3">
              ▸ Email — the best way
            </div>
            <button
              onClick={copy}
              ...
              {[
                ...
                <li key={l.label}>
                  <a href="#" className="group/a flex items-center gap-3 text-[16px] text-[var(--ink)]/85 hover:text-[var(--pink)] transition-colors">
                    ...
                    <span className="font-hand text-[16px] text-[var(--ink-muted)] opacity-0 group-hover/a:opacity-100 transition-opacity ml-2">
                      — {l.h}
                    </span>
                  </a>
                </li>
    ...
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="pt-8 border-t border-[var(--border)] flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
        <div className="flex flex-col gap-1">
          ...
          <div className="font-hand text-[16px] text-[var(--ink-muted)]">
            built this site myself, obviously
          </div>
        </div>
        ...
        <div className="flex items-center gap-6 font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--ink-subtle)]">
          ...
          <a href="#" className="hover:text-[var(--ink)] transition-colors">Imprint</a>
          <a href="#" className="hover:text-[var(--ink)] transition-colors">RSS</a>
        </div>
      </div>
    </div>
  </footer>
...
/* ============================================================
   PAGE
   ============================================================ */
export default function JoachimSite() {
  return (
